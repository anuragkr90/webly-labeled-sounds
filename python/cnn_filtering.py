...
def setupClassifier(training_input_directory,validation_input_directory,testing_input_directory,classCount,spec_count,segment_length,learning_rate,output_path):
	...
	#scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.87)
        ...
        if use_cuda:
            ...
            if args.preloading:
                load_model_direct(net,args.preloading_model_path)
	else:
            if args.preloading:
	...
	model_path=''
        for epoch in range(args.num_epochs):
            ...
            if epoch%10 == 0:
                ...
                seg_train, seg_labels = netFilterStack(net,training_input_directory,args.selK,args.segsize,args.hopsize)
...
def netFilterStack(net,training_input_directory,selK,segsize,hopsize):
    ...
    for i in range(len(args.train_list)):
        ...
        for batch in dataloader(training_input_directory,training_files,1,feat_dim,shuffle=False,last_batch=True):
            ...
            for cl in np.where(targets)[1]:
                ...
                if cres_sort_id.shape[0] >= selK:
                    segids = cres_sort_id[0:selK]
                else:
                    segids = cres_sort_id
                    
                if segids.tolist() in allcres:
                    findid = allcres.index(segids)
                    all_lb_id = lbset[findid]
                    seglb = all_labels[all_lb_id,:]
                    seglb[cl] = 1.0
                    all_labels[all_lb_id,:] = seglb
                else:
                    allcres.append(segids.tolist())
                    
                    selKdata = indata[segids,:,:,:]
                    selKdata = np.expand_dims(np.squeeze(selKdata),0)
                    if len(selKdata.shape) < 3:
                        selKdata = np.expand_dims(selKdata,0)
                
                    selKlabels = np.zeros(inres.shape[1])
                    selKlabels[cl] = 1.0
                    all_predictions.append(selKdata)
                    all_labels.append(selKlabels)

                    lbset.append(len(all_labels))
                #print len(all_predictions), len(all_labels)
