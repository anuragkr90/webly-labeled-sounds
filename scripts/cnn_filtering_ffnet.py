...
def setupClassifier(training_input_directory,validation_input_directory,testing_input_directory,classCount,spec_count,segment_length,learning_rate,output_path):
    ...
    if args.newnet == 'newNET':
        net = newNET(classCount,feat_train.shape[1])
    elif args.newnet == 'newNET1':
        net = newNET1(classCount,feat_train.shape[1])
    ...
    for epoch in range(args.num_epochs):
        ...
        #train on filtered
        netFeatTrain(net,(feat_train,feat_train_labels),optimizer,loss_fn,epoch)

        #validation
        start_time_val = time.time()
        aps, aucs, aps_ranked, epo_val_loss = netFeatVald(net,(feat_vald,feat_vald_labels),loss_fn)
    ...
    aps, aucs, aps_ranked, epo_test_loss = netFeatVald(net,(feat_test,feat_test_labels),loss_fn)
